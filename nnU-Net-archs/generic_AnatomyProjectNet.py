...
class ChannelSELayer3D(nn.Module):
    ...
    def forward(self, input_tensor):
        ...
        print("This is the input_tensor in ChannelSELayer3D ", np.shape(input_tensor))
        # Average along each channel
        squeeze_tensor = self.avg_pool(input_tensor)
        print("This is the squeeze_tensor in ChannelSELayer3D ", np.shape(squeeze_tensor))

        # channel excitation
        fc_out_1 = self.relu(self.fc1(squeeze_tensor.view(batch_size, num_channels)))
        print("This is the fc_out_1 in ChannelSELayer3D ", np.shape(fc_out_1))

        fc_out_2 = self.sigmoid(self.fc2(fc_out_1))
        print("This is the fc_out_2 in ChannelSELayer3D ", np.shape(fc_out_2))

        output_tensor = torch.mul(input_tensor, fc_out_2.view(batch_size, num_channels, 1, 1, 1))
        dummy = fc_out_2.view(batch_size, num_channels, 1, 1, 1)
        print("This is the dummy in ChannelSELayer3D ", np.shape(dummy))
        print("This is the output_tensor in ChannelSELayer3D ", np.shape(output_tensor))
...
class SpatialSELayer3D(nn.Module):
    ...
    def forward(self, input_tensor, weights=None):
        ...
        print("This is the squeeze_tensor in SpatialSELayer3D ", np.shape(squeeze_tensor))

        # spatial excitation
        output_tensor = torch.mul(input_tensor, squeeze_tensor.view(batch_size, 1, D, H, W))

        dummy = squeeze_tensor.view(batch_size, 1, D, H, W)
        print("This is the dummy which is re-viewed in SpatialSELayer3D ", np.shape(dummy))
        print("This is the input_tensor in SpatialSELayer3D ", np.shape(input_tensor))
        print("This is the output_tensor in SpatialSELayer3D ", np.shape(output_tensor))
...
class ChannelSpatialSELayer3D(nn.Module):
    ...
    def forward(self, input_tensor):
        ...
        print("here 2")
        print("This is the sSE in ChannelSpatialSELayer3D ", np.shape(self.sSE(input_tensor)))

        print("here 3")
        print("This is the cSE in ChannelSpatialSELayer3D ", np.shape(self.cSE(input_tensor)))

        print("here 4")
        output_tensor = torch.max(self.cSE(input_tensor), self.sSE(input_tensor))

        print("here 5")
        print("This is the output_tensor in ChannelSpatialSELayer3D ", np.shape(output_tensor))
...
class ProjectExciteLayer(nn.Module):
    ...
    def forward(self, input_tensor):
        ...
        #print("check this mate! ", final_squeeze_tensor.shape)
...
class SEBasicBlock3D(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, reduction=15):
        ...
        self.bn2 = nn.BatchNorm3d(planes)
        self.se = ProjectExciteLayer(planes) # PK SELayer3D
        #self.se = ChannelSpatialSELayer3D(planes, reduction) #SpatialSELayer3D(planes)
        if inplanes != planes:
            self.downsample = nn.Sequential(nn.Conv3d(inplanes, planes, kernel_size=1, stride=stride, bias=False),
                                            nn.BatchNorm3d(planes))
        else:
    ...
    def forward(self, x):
        ...
        out = self.bn1(out)
        out = self.relu(out)
        print("SEBasicBlock3D internal: ", out.shape)
...
#         if self.downsample is not None:
        residual = self.downsample(x)
        print("SEBasicBlock3D internal residual: ", residual.shape)
        out += residual
        out = self.relu(out)
        print("SEBasicBlock3D internal: ", out.shape)
        return out
...
class UpSEBasicBlock3D(nn.Module):
    def __init__(self, inplanes1, inplanes2, planes, stride=1, downsample=None, reduction=16):
        ...
        self.bn2 = nn.BatchNorm3d(planes)
        self.se = ProjectExciteLayer(planes) ## PK SELayer3D
        #self.se = ChannelSpatialSELayer3D(planes, reduction) #SpatialSELayer3D(planes) #,reduction
        if inplanes3 != planes:
...
#         print(x1.size(), x2.size())
        ...
        print("UpSEBasicBlock3D internal: ", out.shape)
        residual = self.downsample(out)
        print("UpSEBasicBlock3D internal residual: ", residual.shape)
...
#         print(x1.size(), x2.size())
        ...
        print("UpBasicBlock3D internal: ", out.shape)
        residual = self.downsample(out)
        print("UpBasicBlock3D internal residual: ", out.shape)
        #print(out.size(), residual.size())
        out = self.conv1(out)
        out = self.bn1(out)
        out = self.relu(out)
...
class generic_AnatomyProjectNet(SegmentationNetwork):
    print("check: Im using the generic_AnatomySENet network architecture")
    ...
    def __init__(self, input_channels, base_num_features, num_classes, num_pool, num_conv_per_stage=2,
        ...
        super(generic_AnatomyProjectNet, self).__init__()
        ...
        #print("self.layer11", self.layer11.shape)
